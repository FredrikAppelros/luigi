...
try:
    ...
    logger.warning("Loading vertica module without pyodbc installed. Will crash at runtime if vertica functionality is used.")
...
class VerticaTarget(luigi.Target):
    ...
    def __init__(
        self, host, database, user, password, table, update_id, port=5433
    ):
        """
        Args:
...
class CopyToTable(rdbms.CopyToTable):
    ...
    def copy(self, cursor, file_):
        ...
        cursor.execute(
            "COPY {} ({}) FROM LOCAL '{}' DELIMITER '{}'".format(
                self.table, ','.join(column_names), file_.name, self.column_separator
            )
        )
    ...
    def run(self):
        ...
        with tempfile.NamedTemporaryFile(dir=tmp_dir) as tmp_file, self.output().connect() as connection:
            ...
            for attempt in range(2):
                try:
                    ...
                    if e.args[0] == '42V01' and attempt == 0:
